...
import co.sideralis.core.challenge.commands.UpdateChallengeName;
...
@CommandHandles
@CommandType(name = "core.challenge.updatechallengename", aggregate = "challenge")
public class UpdateChallengeNameHandle  extends UseCaseExecutor {
    private RequestCommand<UpdateChallengeName> request;

    @Override
    public void run() {
        runUseCase(new UseCase<>() {
            @Override
            public void executeUseCase(RequestCommand<UpdateChallengeName> requestCommand) {
                ...
                challenge.updateChallengeName(command.getNewName());
    ...
    public void accept(Map<String, String> args) {
        request = new RequestCommand<>(
                new UpdateChallengeName(
                        ChallengeId.of(aggregateId()),
                        new Name(args.get("newName"))
                )
        );
    }
}
